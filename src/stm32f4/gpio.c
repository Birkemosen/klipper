...
gpio_in_setup(uint8_t pin, int8_t pull_up)
{
    ...
    if (pull_up) {
        ...
        LL_GPIO_SetPinMode(regs, bit, LL_GPIO_MODE_INPUT);
        LL_GPIO_SetPinPull(regs, bit, LL_GPIO_PULL_NO);
    }
    irq_restore(flag);
...
gpio_adc_sample(struct gpio_adc g)
{
    ...
    if (!readb(&adc_busy)) {
        ...
        LL_ADC_SetChannelSamplingTime(ADC1, g.bit, LL_ADC_SAMPLINGTIME_480CYCLES);
        LL_ADC_REG_StartConversionSWStart(ADC1);
        adc_busy = true;
        adc_current_channel = g.bit;
        return ADC_DELAY;
    /* ADC finished conversion for this channel */
    } else if (LL_ADC_IsActiveFlag_EOCS(ADC1) &&
